...
if(isset($_POST['submit'])){
    ...
    $result = mysqli_query($conn,"SELECT * FROM `tb_donatur` WHERE email ='$email' ");
    $row=mysqli_fetch_assoc($result);
    $password = $_POST['password'];
    if($_POST['passwordl'] == $row['password'] && $_POST['password'] == $_POST['cpassword'] ) {
    mysqli_query($conn,"UPDATE `tb_donatur` SET `password` = '$password' WHERE `tb_donatur`.`email` = '$email'");
