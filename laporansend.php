...
<body>
    <div style="padding: 15px;">
        ...
        <hr />
        <h3 style="margin-top: 0;"><b>Laporan Donasi dikirim ke posko</b></h3>
        ...
        if(empty($tgl_awal) or empty($tgl_akhir)){ // Cek jika tgl_awal atau tgl_akhir kosong, maka :
            ...
            $query = "SELECT * FROM tb_tdonasi WHERE tb_tdonasi.status = '1' ";
            $url_cetak = "printsend.php";
            $label = "Semua Data Donasi";
        }else{ // Jika terisi
            // Buat query untuk menampilkan data transaksi sesuai periode tanggal
            $query = "SELECT * FROM tb_tdonasi  WHERE tb_tdonasi.status = '1' AND (waktu_donasi BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."')";
            $url_cetak = "printsend.php?tgl_awal=".$tgl_awal."&tgl_akhir=".$tgl_akhir."&filter=true";
